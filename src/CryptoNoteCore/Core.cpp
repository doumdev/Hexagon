...
std::error_code Core::addBlock(const CachedBlock& cachedBlock, RawBlock&& rawBlock) {
  ...
  if (previousBlockIndex <= 6357) rewardBlocksWindow = 50; // Hexagon historical quirk
  ...
  if (minerReward != reward && previousBlockIndex >= CryptoNote::parameters::HEXAGON_REWARD_ADJUSTMENT_BLOCK) { // permissive for Hexagon's history
  ...
  if (checkpoints.isInCheckpointZone(cachedBlock.getBlockIndex())) {
    ...
    // Hexagon hack -- between blocks 6358 and 8550 difficulty checks were turned off (bug)
