...
#ifdef _WIN32
const std::string hexagonDaemon = "hexagond.exe";
#else
const std::string hexagonDaemon = "hexagond";
#endif
...
TestNodeConfiguration TestNetworkBuilder::buildNodeConfiguration(size_t index) {
  ...
  cfg.daemonPath = hexagonDaemon; // default
  cfg.testnet = testnet;
  cfg.logFile = "test_hexagond" + std::to_string(index) + ".log";
